resource "fastly_service_v1" "app" {
  name = "Origami Build Service v3 Production"

  domain {
    name = "origami-build-service-v3.in.ft.com"
  }

  backend {
    name                  = "eu"
    address               = "w1n79xzb6h.execute-api.eu-west-1.amazonaws.com"
    port                  = 443
    ssl_cert_hostname     = "*.execute-api.eu-west-1.amazonaws.com"
    ssl_sni_hostname      = "w1n79xzb6h.execute-api.eu-west-1.amazonaws.com"
  ...
  backend {
    name                  = "us"
    address               = "ms4bwwwuaj.execute-api.us-west-1.amazonaws.com"
    port                  = 443
    ssl_cert_hostname     = "*.execute-api.us-west-1.amazonaws.com"
    ssl_sni_hostname      = "ms4bwwwuaj.execute-api.us-west-1.amazonaws.com"
    auto_loadbalance      = false
    connect_timeout       = 5000
    first_byte_timeout    = 120000
    between_bytes_timeout = 120000
  ...
  header {
    ...
    source      = "\"w1n79xzb6h.execute-api.eu-west-1.amazonaws.com\""
  }
  ...
  header {
    ...
    source      = "\"ms4bwwwuaj.execute-api.us-west-1.amazonaws.com\""
  }

  vcl {
    name    = "prepend_path_with_lambda_stage.vcl"
    content = "sub prepend_path_with_lambda_stage {set bereq.url = \"/prod\" bereq.url;}"
  }
