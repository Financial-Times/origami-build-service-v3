...
sub set_backend {
	...
	# macro has the code which defines the values available for req.http.Host
	# but it also contains a default backend which is always set to the EU. 
	# I wish we could disable the default backend setting.
	#FASTLY recv
	...
	# Route EU requests to the nearest healthy shield or origin.
  	if (var.region == "EU") {
		if (server.datacenter != var.EU_shield_server_name && req.http.Request_Came_From_Shield != var.EU_shield_server_name && var.shield_eu_is_healthy) {
			...
			# Everything is on fire... but lets 
			# try the origin anyway just in case
			# it's the healthchecks that are wrong
			set req.backend = F_eu_without_healthcheck;
			set req.http.Host = req.http.EU_Host;
		}
  	}
	...
	# Route US requests to the nearest healthy shield or origin.
  	if (var.region == "US") {
		if (server.datacenter != var.US_shield_server_name && req.http.Request_Came_From_Shield != var.US_shield_server_name && var.shield_us_is_healthy) {
			...
			# try the origin anyway just in case
			# it's the healthchecks that are wrong
			set req.backend = F_us_without_healthcheck;
			set req.http.Host = req.http.US_Host;
		}
	}
...
sub vcl_recv {
	...
	set req.url = regsub(req.url, "^/__origami/service/build","");
	set req.http.FT-Origami-Service-Base-Path = "/__origami/service/build/";
...
sub vcl_hash {
	...
	# Do not add the host header because it can have 1 of 3 values: origami-build-service-v3.in.ft.com, api-gateway eu, api-gateway us
