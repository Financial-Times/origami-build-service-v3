...
const sanitiseShrinkwrapParameter = require("./middleware/sanitise-shrinkwrap-parameter");
...
module.exports = async function service(options) {
	await createNetRcFile(options);
...
async function createNetRcFile(options) {
	/**
	 * This file is needed to access our private Origami components. E.G. o-fonts-assets
	 */
	const tempFolder = await createTempFolder();
	await fs.writeFile(
		join(tempFolder, "/.netrc"),
		`machine github.com\nlogin ${options.githubUsername}\npassword ${
			options.githubPassword
		}`,
	);
	process.env.HOME = tempFolder; // Workaround: Bower ends up using $HOME/.local/share/bower/empty despite config overriding this
}
