...
const sanitiseBrandParameter = require("./middleware/sanitise-brand-parameter");
...
module.exports = function service(options) {
	...
	if (options.environment !== "production") {
		if (!"requestLogFormat" in options) {
...
function mountRoutes(app) {
	...
	app.get("/v3/api", (request, response) => {
		response.render("api", {
			title: app.ft.options.name,
			heading: "Origami Registry",
		});
	});
	app.get("/v3/url-builder", (request, response) => {
